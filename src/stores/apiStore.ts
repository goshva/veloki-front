...
export const useApiStore = defineStore('api', () => {
  ...
  const withLoading = async (operation: () => Promise<any>, dataType: keyof typeof dataLoaded.value) => {
    loading.value = true
  ...
  async function updateOrder(orderId: number, orderData: Partial<OrderData>) {
    return withLoading(async () => {
      const response = await axios.put(`${apiBaseUrl}/orders/${orderId}`, orderData)
